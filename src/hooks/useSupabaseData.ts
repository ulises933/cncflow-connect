...
export const useCreateEntrega = () => {
  ...
  return useMutation({
    mutationFn: async (values: any) => {
      const { data, error } = await supabase.from("entregas").insert(values).select().single();
      if (error) throw error;
      return data;
    },
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["entregas"] }); toast.success("Entrega registrada"); },
    onError: (e: Error) => toast.error(e.message),
  });
};
...
export const useUpdateEntrega = () => {
  ...
  return useMutation({
    mutationFn: async ({ id, ...values }: { id: string; [key: string]: any }) => {
      const { error } = await supabase.from("entregas").update(values).eq("id", id);
      if (error) throw error;
    },
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["entregas"] }); toast.success("Entrega actualizada"); },
    onError: (e: Error) => toast.error(e.message),
  });
};
